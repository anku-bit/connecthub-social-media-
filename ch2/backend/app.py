...
Complete Flask REST API - all bugs fixed
"""
from flask import Flask, request, jsonify, send_from_directory
from flask_cors import CORS
import os, re, datetime, functools, bcrypt, jwt

from database import get_db, init_db

app = Flask(__name__, static_folder="../frontend/public", static_url_path="")
CORS(app, origins="*")
SECRET = os.environ.get("SECRET_KEY", "connecthub_2024_secret")


# ─── helpers ──────────────────────────────────────────────────────────────────

def hp(p):     return bcrypt.hashpw(p.encode(), bcrypt.gensalt()).decode()
def chkp(p,h): return bcrypt.checkpw(p.encode(), h.encode())
...
def get_viewer_id():
    ...
    if tok.startswith("Bearer "):
        d = decode_token(tok[7:])
        if d: return d.get("uid", 0)
    return 0

def notify(to_uid, actor_id, ntype, message, post_id=None):
    if to_uid == actor_id: return
    try:
        conn = get_db()
        conn.execute(
...
def pub_user(u):
    return {k: v for k, v in (dict(u) if not isinstance(u, dict) else u).items() if k != "password"}
...
def register():
    ...
    if not re.match(r"^[a-z0-9_.]{3,20}$", username):
        return jsonify({"error": "Username: 3-20 chars, letters/numbers/dot/underscore only"}), 400
    ...
    conn = get_db()
    # Check duplicates manually for clear error messages
    ...
    try:
        ...
        return jsonify({"token": make_token(uid), "user": pub_user(user)}), 201
...
def login():
    ...
    conn = get_db()
    u = conn.execute("SELECT * FROM users WHERE email=? OR username=?", (em, em)).fetchone()
    conn.close()
    if not u or not chkp(pw, u["password"]):
        return jsonify({"error": "Invalid credentials"}), 401
    return jsonify({"token": make_token(u["id"]), "user": pub_user(u)})
...
def update_profile():
    ...
    conn.execute(
        ...
        (safe(d.get("name", cur["name"])),
         safe(d.get("bio",  cur["bio"] or ""), 500),
         safe(d.get("location", cur["location"] or "")),
         safe(d.get("website",  cur["website"]  or "")),
         u["id"])
    )
    conn.commit()
    updated = conn.execute("SELECT * FROM users WHERE id=?", (u["id"],)).fetchone()
    conn.close()
    return jsonify(pub_user(updated))
...
def feed():
    ...
    rows = conn.execute("""
        SELECT p.*,
               u.name, u.username, u.is_verified,
...
def create_post():
    ...
    conn.commit()
    post = conn.execute("""
        SELECT p.*, u.name, u.username, u.is_verified
...
def delete_post(pid):
    ...
    if not p:
        conn.close(); return jsonify({"error": "Post not found"}), 404
    if p["user_id"] != u["id"]:
        conn.close(); return jsonify({"error": "Not your post"}), 403
...
def toggle_like(pid):
    ...
    conn = get_db()
    p = conn.execute("SELECT user_id FROM posts WHERE id=?", (pid,)).fetchone()
    if not p: conn.close(); return jsonify({"error": "Post not found"}), 404
    ex = conn.execute("SELECT id FROM likes WHERE user_id=? AND post_id=?", (u["id"], pid)).fetchone()
    if ex:
        ...
        liked = True
        notify(p["user_id"], u["id"], "like", f"{u['name']} liked your post", post_id=pid)
...
def add_comment(pid):
    ...
    conn.commit()
    notify(p["user_id"], u["id"], "comment", f"{u['name']} commented on your post", post_id=pid)
    cm = conn.execute("""
        SELECT c.*, u.name, u.username
...
def delete_comment(cid):
    ...
    if not cm: conn.close(); return jsonify({"error": "Comment not found"}), 404
    if cm["user_id"] != u["id"]: conn.close(); return jsonify({"error": "Not your comment"}), 403
    conn.execute("DELETE FROM comments WHERE id=?", (cid,))
    conn.execute("UPDATE posts SET comment_count=MAX(0,comment_count-1) WHERE id=?", (cm["post_id"],))
    conn.commit(); conn.close()
...
def toggle_follow(target_id):
    ...
    ex = conn.execute(
        "SELECT id FROM follows WHERE follower_id=? AND following_id=?", (u["id"], target_id)
    ).fetchone()
    if ex:
        conn.execute("DELETE FROM follows WHERE follower_id=? AND following_id=?", (u["id"], target_id))
        ...
        conn.execute("INSERT INTO follows(follower_id,following_id) VALUES(?,?)", (u["id"], target_id))
        ...
        following = True
        tuser = conn.execute("SELECT name FROM users WHERE id=?", (target_id,)).fetchone()
        notify(target_id, u["id"], "follow", f"{u['name']} started following you")
    fc = conn.execute("SELECT follower_count FROM users WHERE id=?", (target_id,)).fetchone()["follower_count"]
    conn.commit(); conn.close()
...
def get_followers(uid):
    ...
    conn   = get_db()
    rows   = conn.execute("""
        SELECT u.id,u.name,u.username,u.bio,u.is_verified,u.follower_count,
...
def get_profile(username):
    ...
    is_following = bool(conn.execute(
        "SELECT 1 FROM follows WHERE follower_id=? AND following_id=?", (viewer, user["id"])
    ).fetchone())
    posts = conn.execute("""
        SELECT p.*,
               (SELECT 1 FROM likes WHERE user_id=? AND post_id=p.id) AS is_liked,
...
def toggle_save(pid):
    u    = request.user
    conn = get_db()
    ex   = conn.execute("SELECT id FROM saved_posts WHERE user_id=? AND post_id=?", (u["id"], pid)).fetchone()
    if ex:
        conn.execute("DELETE FROM saved_posts WHERE user_id=? AND post_id=?", (u["id"], pid))
        saved = False
    else:
        conn.execute("INSERT INTO saved_posts(user_id,post_id) VALUES(?,?)", (u["id"], pid))
        saved = True
    conn.commit(); conn.close()
...
def saved_posts():
    ...
    conn = get_db()
    rows = conn.execute("""
        SELECT p.*, u.name, u.username, u.is_verified, 1 AS is_saved,
               (SELECT 1 FROM likes WHERE user_id=? AND post_id=p.id) AS is_liked
        FROM saved_posts sp
...
def search():
    ...
    if len(q) < 2: return jsonify({"users": [], "posts": []})
    viewer = get_viewer_id()
    conn   = get_db()
    users  = conn.execute("""
        SELECT id,name,username,bio,is_verified,follower_count,
               (SELECT 1 FROM follows WHERE follower_id=? AND following_id=users.id) AS is_following
        FROM users WHERE name LIKE ? OR username LIKE ? OR bio LIKE ? LIMIT 8
    """, (viewer, f"%{q}%", f"%{q}%", f"%{q}%")).fetchall()
    posts  = conn.execute("""
        SELECT p.*, u.name, u.username, u.is_verified,
               (SELECT 1 FROM likes WHERE user_id=? AND post_id=p.id) AS is_liked
        FROM posts p JOIN users u ON p.user_id=u.id
        WHERE p.content LIKE ? ORDER BY p.like_count DESC LIMIT 10
    """, (viewer, f"%{q}%")).fetchall()
    conn.close()
...
def trending():
    conn  = get_db()
    tags  = conn.execute("SELECT * FROM hashtags ORDER BY post_count DESC LIMIT 10").fetchall()
    users = conn.execute("""
        SELECT id,name,username,bio,follower_count,is_verified
        FROM users WHERE username != 'admin'
        ORDER BY follower_count DESC LIMIT 8
    """).fetchall()
    conn.close()
    return jsonify({"hashtags": [dict(r) for r in tags], "users": [dict(r) for r in users]})
...
def people():
    ...
    rows   = conn.execute("""
        SELECT id,name,username,bio,follower_count,is_verified,
               (SELECT 1 FROM follows WHERE follower_id=? AND following_id=users.id) AS is_following
        FROM users WHERE username != 'admin' ORDER BY follower_count DESC
    """, (viewer,)).fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
if __name__ == "__main__":
    ...
    print("  admin@connecthub.com  /  Admin@123")
    print("=" * 52 + "\n")
